...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.delete_unique(u'djiki_page', ['title'])

        # Adding field 'Page.language'
        # *************************************************************************
        #
        # All existing pages will have the language code initialized with the value
        # of LANGUAGE_CODE setting.
        #
        # *************************************************************************
        db.add_column(u'djiki_page', 'language',
                      self.gf('django.db.models.fields.CharField')(
                          default=getattr(settings, 'LANGUAGE_CODE', ''),
                          max_length=5, blank=True),
                      keep_default=False)

        # Adding unique constraint on 'Page', fields ['language', 'title']
        db.create_unique(u'djiki_page', ['language', 'title'])


    def backwards(self, orm):
        # Removing unique constraint on 'Page', fields ['language', 'title']
        db.delete_unique(u'djiki_page', ['language', 'title'])

        # Deleting field 'Page.language'
        db.delete_column(u'djiki_page', 'language')

        # Adding unique constraint on 'Page', fields ['title']
        db.create_unique(u'djiki_page', ['title'])
    ...
    models = {
        ...
        u'djiki.page': {
            'Meta': {'ordering': "('title',)", 'unique_together': "(('title', 'language'),)", 'object_name': 'Page'},
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
